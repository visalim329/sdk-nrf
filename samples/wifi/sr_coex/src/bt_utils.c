...
void disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	if (info.role == BT_CONN_ROLE_CENTRAL) {
		...
		scan_start(); // dont use for central.
...
void bt_conn_test_run(void)
{
	...
	while (true) {
		...
		/* Less than 2 seconds of sleep time throws coredump errors. */
		k_sleep(K_SECONDS(2));
	}
}
...
int bt_throughput_test_init(bool is_ble_central)
{
	...
	while (k_uptime_delta(&stamp) / MSEC_PER_SEC < THROUGHPUT_CONFIG_TIMEOUT) {
		...
		k_sleep(K_SECONDS(1)); /* this is before test start. No need to reduce further. */
	}
